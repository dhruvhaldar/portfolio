import "@/once-ui/styles/index.scss";import "@/once-ui/tokens/index.scss";import classNames from "classnames";import { Header, RouteGuard } from "@/components";import dynamic from "next/dynamic";import { baseURL, effects, style } from "@/app/resources";
import localFont from 'next/font/local';
import { person, home } from "@/app/resources/content";import { Background, Column, Flex, ToastProvider } from "@/once-ui/components";import { SpeedInsights } from "@vercel/speed-insights/next";
const Footer = dynamic(() => import('@/components/Footer').then((mod) => mod.Footer),{ loading: () => <div>Loading Footer...</div> });export async function generateMetadata() {const title = home.title;const alt_title = 'Showcasing the Best Projects';const description = home.description;const ogImage = `https://${baseURL}/opengraph.jpg`;const pageUrl = `https://${baseURL}`;return {title,description,metadataBase: new URL(pageUrl),alternates: {canonical: pageUrl},icons: {icon: [{ url: '/favicon.ico', sizes: '48x48' },{ url: '/favicon-16x16.png', sizes: '16x16', type: 'image/png' },{ url: '/favicon-32x32.png', sizes: '32x32', type: 'image/png' },{ url: '/android-chrome-192x192.jpg', sizes: '192x192', type: 'image/jpeg' },{ url: '/android-chrome-512x512.jpg', sizes: '512x512', type: 'image/jpeg' },],shortcut: "/favicon.ico",apple: "/apple-icon.png",manifest: '/site.webmanifest',other: {rel: "apple-touch-icon",url: "/apple-touch-icon-precomposed.png",'msapplication-TileColor': '#da532c','theme-color': '#ffffff',},},openGraph: {title,description,type: "website",url: pageUrl,siteName: `${person.firstName}'s Portfolio (With Projects & Publications)`,locale: "en_US",images: [{url: ogImage,width: 1200,height: 630,alt: alt_title,},],},twitter: {card: "summary_large_image",title,description,images: [ogImage],},robots: {index: true,follow: true,googleBot: {index: true,follow: true,"max-video-preview": -1,"max-image-preview": "large","max-snippet": -1,},},};}
