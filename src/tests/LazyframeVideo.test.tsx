...
describe("LazyframeVideo", () => {
  it("should render for valid YouTube URLs", () => {
    const { container } = render(
      <LazyframeVideo src="https://www.youtube.com/watch?v=dQw4w9WgXcQ" />,
    );
  ...
  it("should return null for URLs containing youtube pattern but hosted elsewhere", () => {
    ...
    const { container } = render(
      <LazyframeVideo src="https://evil.com/fake?u=youtube.com/watch?v=dQw4w9WgXcQ" />,
    );
    expect(container.firstChild).toBeNull();
  });

  it("should strip malicious query parameters from data-src", () => {
    const maliciousSrc = "https://www.youtube.com/watch?v=dQw4w9WgXcQ&evil=alert(1)";
    const { container } = render(<LazyframeVideo src={maliciousSrc} />);
    const lazyframeDiv = container.querySelector(".lazyframe");

    // It should be the canonical URL, stripping the evil param
    expect(lazyframeDiv).toHaveAttribute("data-src", "https://www.youtube.com/watch?v=dQw4w9WgXcQ");
  });
});
